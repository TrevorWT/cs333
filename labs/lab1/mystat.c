...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (lstat(argv[i], &sb) == -1) {
      ...
      if (sb.st_mode & S_ISUID) str[3] = (str[3]=='x') ? 's' : 'S';
      if (sb.st_mode & S_ISGID) str[6] = (str[6]=='x') ? 's' : 'S';
      if (sb.st_mode & S_ISVTX) str[9] = (str[9]=='x') ? 't' : 'T';
      ...
      printf("  %-26s%s", "Last file access:", ctime(&sb.st_atime));
      printf("  %-26s%s", "Last file modification:", ctime(&sb.st_mtime));
      printf("  %-26s%s", "Last status change:", ctime(&sb.st_ctime));
    }
  }
  return 0;
}