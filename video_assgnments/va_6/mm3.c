...
float **alloc_matrix(void){
    ...
    int i = -1;
    for (i = 0; i < dim; i++) { mat[i] = malloc(dim * sizeof(float)); }
...
int main(int argc, char *argv[]) {
   ...
   
    {
        ...
        while ((opt = getopt(argc, argv, "t:d:h"))!= -1) {
            ...
            case 'h':
                printf("%s: -t # -d #\n", argv[0]);
                printf("\t-t #: number of threads (ignored)\n");
...

    {
        ...
        double up_time = elapse_time(&et3, &et4);
        double td_time = elapse_time(&et4, &et5);

        printf("Total time: %8.2lf\n", total_time);
        printf("  Allocation time: %8.2lf\n", alloc_time);
        printf("  Initialization time: %8.2lf\n", init_time);   
        printf("  Computation time: %8.2lf\n", comp_time);
        printf("  Update time: %8.2lf\n", up_time);
        printf("  Total data transfer time: %8.2lf\n", td_time);
    }
    return EXIT_SUCCESS;
}